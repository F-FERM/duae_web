...
export default function ServicesListingPage() {
  ...
  const fetchServices = async () => {
    try {
      ...
      const sorted = data.sort(
        (a: ServiceItem, b: ServiceItem) => (a.order || 0) - (b.order || 0),
      );
      setServices(sorted);
    } catch (err: any) {
  ...
  const openCreateModal = () => {
    setEditingId(null);
    setForm({ ...EMPTY_SERVICE, order: services.length, inlineLinks: [] });
    setModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...restForm,
        slug:
          form.slug || form.title.toLowerCase().replace(/[^a-zA-Z0-9]+/g, "-"),
  ...
  const moveService = async (index: number, direction: "up" | "down") => {
    ...
    try {
      const updatePromises = updatedServices.map((service) => {
        if (service._id) {
          return api.patch(`/services/${service._id}`, {
            order: service.order,
          });
        }
  ...
  // Check if service has inline links
  const hasLinks = (service: ServiceItem) => {
    return service.inlineLinks && service.inlineLinks.length > 0;
  };
              ...
              {filteredServices.map((service, index) => (
                ...
                >
                  ...
                  <div className="relative aspect-[16/10] w-full overflow-hidden bg-[#F1E4D8] xs:h-[140px] xs:aspect-auto sm:h-[160px] lg:h-[170px]">
                    ...
                    {/* Show inline links indicator */}
                    {hasLinks(service) && (
                      <div className="absolute bottom-[10px] left-[10px] flex items-center gap-[4px] rounded-full bg-[#EA580C]/90 px-[8px] py-[3px] text-[9px] font-medium text-white backdrop-blur-sm sm:bottom-[12px] sm:left-[12px] sm:px-[10px] sm:py-[4px] sm:text-[10px]">
                        <Hash className="h-[10px] w-[10px] sm:h-[11px] sm:w-[11px]" />
                        {service.inlineLinks?.length} link
                        {service.inlineLinks?.length !== 1 ? "s" : ""}
                      </div>
                    )}
                    ...
                    <div className="mt-[10px] flex items-center gap-[6px] text-[10px] text-[#999] sm:mt-[12px] sm:text-[11px]">
                      ...
                      {hasLinks(service) && (
                        <span className="flex items-center gap-1 text-[#EA580C]">
                          <CircleDot className="h-[8px] w-[8px] fill-[#EA580C]" />
                          {service.inlineLinks?.length} link
                          {service.inlineLinks?.length !== 1 ? "s" : ""}
                        </span>
                      )}
          ...
          >
            ...
            >
              ...
              <div className="mt-[18px] space-y-[14px] sm:mt-[22px] sm:space-y-[16px]">
                ...
                {/* ================= INLINE LINKS ================= */}
                <div className="border-t border-[#E4E4E4] pt-[16px] sm:pt-[20px]">
                  <InlineLinkManager
                    ...
                    label="Inline Links"
                    description="Text within this service content that will become clickable."
                  />
                </div>
