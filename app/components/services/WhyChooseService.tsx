...
interface WhyChooseUsData {
  title: string;
  items: WhyChooseUsItemApi[];
  whyChooseUsInlineLinks?: InlineLink[];
}
...
const defaultData: WhyChooseUsData = {
  title: "Why Choose Us?",
  whyChooseUsInlineLinks: [],
  items: [
    {
      title: "Expertise You Can Trust",
      description:
        "With years of experience, we are recognized as best fit out contractors in Dubai, delivering projects with precision and excellence.",
      icon: "fa-solid fa-star",
      image: "",
      inlineLinks: [],
    },
    {
      title: "Comprehensive Fit-Out Solutions",
      description:
        "As leading interior fit out contractors in Dubai, we handle every aspect of your project - from design and joinery to electrical and finishing touches.",
      icon: "fa-solid fa-clipboard-list",
      image: "",
      inlineLinks: [],
    },
    {
      title: "Innovative & Customized Designs",
      description:
        "We create tailor-made interiors that combine style, functionality, and innovation to reflect your unique vision.",
    ...
    },
    {
      ...
      description:
        "Our skilled team ensures high-quality workmanship, attention to detail, and durable results that stand the test of time.",
      icon: "fa-solid fa-hammer",
      image: "",
      inlineLinks: [],
    },
    {
      title: "Timely Delivery & Client Satisfaction",
      description:
        "We prioritize efficiency and seamless project management, ensuring every fit-out is completed on time while exceeding client expectations.",
      ...
      <div className="relative mx-auto max-w-[1350px] px-4">
        ...
        <div className="mt-14 grid grid-cols-1 gap-x-8 gap-y-12 sm:mt-16 sm:grid-cols-2 lg:grid-cols-4 lg:gap-y-16">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="flex flex-col items-center bg-[#0c0c0c] px-6 pb-10 pt-16 sm:px-8 sm:pt-20"
            >
...
export default function WhyChooseUsService({ slug }: { slug: string }) {
    ...
    const fetchServiceDetail = async () => {
      try {
        ...
        setData({
          ...
          items: res.data.whyChooseUs.items.map((item) => ({
            ...item,
            inlineLinks: item.inlineLinks || [],
          })),
          whyChooseUsInlineLinks: res.data.whyChooseUs.inlineLinks || [],
        });
      } catch (err) {
  ...
  // Section-specific links for title
  const sectionLinks = data.whyChooseUsInlineLinks || [];
      ...
      <div className="relative mx-auto max-w-[1350px] px-4">
        ...
        >
          <InlineLinkedText
            text={data.title}
            links={sectionLinks}
            linkClassName="inline-block cursor-pointer font-extrabold text-white underline decoration-white/30 underline-offset-8 transition-all duration-200 hover:text-[#db5e41] hover:decoration-[#db5e41] focus:outline-none focus:ring-2 focus:ring-[#db5e41] focus:ring-offset-2 focus:ring-offset-black rounded"
          />
        </motion.div>
          ...
          {data.items.map((feature, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="mx-auto mt-4 max-w-[280px] text-sm leading-7 text-white/60 sm:text-[18px]">
                  <InlineLinkedText
                    text={feature.description}
                    links={feature.inlineLinks || []}
                    linkClassName="inline-block cursor-pointer font-medium text-white/60 underline decoration-white/30 underline-offset-4 transition-all duration-200 hover:text-[#db5e41] hover:decoration-[#db5e41] focus:outline-none focus:ring-2 focus:ring-[#db5e41] focus:ring-offset-2 focus:ring-offset-[#0c0c0c] rounded"
                  />
                </div>
              </motion.div>
            );
