    ...
    const fetchHeroOurBlogs = async () => {
      try {
        ...
        setData({
          ...
          bgImage: latest?.image
            ? resolveImage(latest.image)
            : defaultData.bgImage,
          inlineLinks: latest?.inlineLinks || [],
        });
      } catch (err) {
  ...
  return (
    <section className="relative mb-19 w-full overflow-hidden">
      ...
      <div className="relative z-10 mx-auto flex min-h-[320px] max-w-[1220px] flex-col items-center justify-center px-5 py-16 text-center sm:min-h-[360px] md:min-h-[420px] md:py-20">
        ...
        <div className="text-5xl font-extrabold leading-tight text-white sm:text-3xl md:text-7xl lg:text-7xl">
          <InlineLinkedText
            text={data.title}
            links={inlineLinks}
            className="inline"
